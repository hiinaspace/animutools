...
def fetch_anilist_metadata_batch(id_batch):
    ...
    """
    query_parts = [
        f"a{id}: Media(id: {id}, type: ANIME) {{ {MEDIA_FIELDS_QUERY} }}"
        for id in id_batch
    ]
...
def download_image(url, anilist_id):
    ...
    local_path = os.path.join(
        IMAGE_CACHE_DIR, f"{anilist_id}{file_extension or '.jpg'}"
    )
...
def create_atlas_and_metadata():
    ...
    id_to_original_index = {
        anilist_id: i for i, anilist_id in enumerate(ordered_anilist_ids)
    }

    # --- 2. Fetch rich data from AniList API in batches ---
    print("\n--- Step 2: Fetching detailed metadata from AniList API ---")
    id_batches = [
        ordered_anilist_ids[i : i + API_BATCH_SIZE]
        for i in range(0, len(ordered_anilist_ids), API_BATCH_SIZE)
    ]
    ...
    for i, info in enumerate(processed_anime_info):
        ...
        studios = [
            node["name"] for node in api_data.get("studios", {}).get("nodes", [])
        ]
