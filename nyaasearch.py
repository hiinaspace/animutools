...
def fetch_and_parse_nyaa_feed(query: str) -> list[dict]:
    ...
    try:
        ...
        if feed.bozo:
            logging.warning(
                f"The RSS feed may be malformed for query '{query}'. Reason: {feed.bozo_exception}"
            )
        ...
        logging.info(
            f"Found {len(torrents)} torrents that are first episodes for query '{query}'."
        )
        return torrents
...
def find_best_match_from_list(
    anime_details: dict, torrent_list: list[dict]
) -> str | None:
    """
    ...
    if best_score >= SIMILARITY_THRESHOLD:
        logging.info(
            f"Broad search match found for '{romaji_title}' with score {best_score}"
        )
...
def find_best_match_targeted(anime_title: str) -> str | None:
    ...
    if not valid_candidates:
        logging.info(
            f"No valid torrents found for '{anime_title}' after similarity check."
        )
        return None
    ...
    logging.info(
        f"Found a fallback match for '{anime_title}': {best_fallback['resolution'] or 'Unknown resolution'}."
    )
...
def main():
    ...
    if not metadata_path.exists():
        logging.error(
            f"Input file not found: '{metadata_path}'. Please ensure it is in the same directory."
        )
        return
    ...
    if broad_search_torrents:
        ...
        logging.warning(
            "Broad search returned no torrents. Proceeding directly to targeted search for all anime."
        )
        unmatched_anime = all_anime

    found_count = len(all_anime) - len(unmatched_anime)
    logging.info(
        f"Broad search complete. Matched {found_count} anime. {len(unmatched_anime)} remain unmatched."
    )
    ...
    if unmatched_anime:
        ...
        for i, anime in enumerate(unmatched_anime):
            ...
            logging.info(
                f"Searching for unmatched anime ({i + 1}/{len(unmatched_anime)}): '{romaji_title}'"
            )
            ...
            if i < len(unmatched_anime) - 1:
                logging.info(
                    f"Waiting for {QUERY_DELAY_SECONDS} seconds before the next query..."
                )
    ...
    try:
        ...
        logging.info(
            f"Successfully wrote results for {final_matched_count}/{len(all_anime)} anime to '{OUTPUT_FILE}'."
        )
    except IOError as e:
