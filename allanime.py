...
import subprocess # Needed for running ffmpeg command
...
async def _handle_ffmpeg_progress(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, handler):
    ...
    try:
        ...
        print("Progress handler task cancelled.", file=sys.stderr)
         ...
         print(f"Progress connection closed unexpectedly ({type(e).__name__}).", file=sys.stderr)
    except Exception as e:
        print(f"Error in progress handler coroutine: {type(e).__name__}: {e}", file=sys.stderr)
...
async def _watch_progress(handler):
    ...
    async def connection_callback(reader, writer):
        ...
        if listen_task and not listen_task.done():
            print("Warning: Received unexpected second connection for progress. Ignoring.", file=sys.stderr)
            writer.close()
            await writer.wait_closed()
            return
        # Create task to handle this specific connection
        listen_task = asyncio.create_task(_handle_ffmpeg_progress(reader, writer, handler))
    ...
    try:
        ...
        async with server: # Ensure server is properly closed
            ...
            print("Progress server yielding control, ffmpeg should connect soon...", file=sys.stderr)
...
async def show_progress(total_duration, desc="Progress"):
    ...
    if total_duration <= 0:
        print(f"Warning: Cannot show progress bar [{desc}] with non-positive total duration ({total_duration:.2f}s).", file=sys.stderr)
        yield None
        return
    ...
    try:
        ...
        def handler(key, value):
            ...
            if key == 'out_time_ms':
                ...
                if bar.n < bar.total: bar.update(bar.total - bar.n)
    ...
    if f.lower().endswith('.mkv'):
         try:
             ...
             if probe.get('streams'):
              ...
              print(f"Warning: Could not probe subtitles for {f}: {e.stderr.decode(errors='ignore')}", file=sys.stderr)
         except Exception as e:
              print(f"Warning: Error probing subtitles for {f}: {e}", file=sys.stderr)

    return ff.filter('scale', int(horiz), int(vert)).filter('setsar','1').filter('format', 'yuv420p')
...
def probe_duration(filename):
    ...
    try:
        ...
        if duration_str:
            ...
            except ValueError: pass # Fall through if format duration is invalid
        ...
        for stream in probe.get('streams', []):
            if stream.get('codec_type') == 'video':
                ...
                if duration_str:
                    try: return float(duration_str)
                    except ValueError: pass # Ignore invalid stream duration
                break # Use first video stream found
        print(f"  Warning: Could not find valid duration in format or video stream for {os.path.basename(filename)}", file=sys.stderr)
        return 0.0
    except ffmpeg.Error as e:
        print(f"  Warning: ffmpeg probe failed for {filename}: {e.stderr.decode(errors='ignore')}", file=sys.stderr)
        return 0.0
    except Exception as e:
...
def get_max_duration(input_files):
    ...
    if max_duration <= 0.0:
         print("Warning: Could not determine maximum duration. Progress bar for Step 1 might be inaccurate or disabled.", file=sys.stderr)
         return 0.0

    print(f"Determined maximum duration for Step 1: {max_duration:.2f}s", file=sys.stderr)
    return max_duration
...
async def encode(input_files, output_file):
    ...
    try:
        ...
        hstacks = [ffmpeg.filter(chunk, 'hstack', inputs=min(columns, len(chunk))) for chunk in chunked(video_inputs, columns)]
        vstack = ffmpeg.filter(hstacks, 'vstack', inputs=len(hstacks))

        audio_inputs_raw = [ffmpeg.input(f).audio for f in input_files if not f.lower().endswith(('.jpg', '.jpeg', '.png', '.gif'))]
        ...
        global_args1 = ['-map_chapters', '-1', '-sn', '-nostdin', '-nostats'] # Base args

        # Use the async show_progress context manager for Step 1
        async with show_progress(duration_step1, desc="Step 1: Encoding Grid") as progress_url1:
            current_global_args = list(global_args1) # Copy base args
            if progress_url1:
                print(f"Using progress URL for Step 1: {progress_url1}", file=sys.stderr)
            ...
            stream1 = ffmpeg.output(*stream1_args, temp_filename, **opts1).global_args(*current_global_args)
            ...
            try:
                ...
                print(f"--- Step 1: FFmpeg process finished. ---", file=sys.stderr)
            except ffmpeg.Error as e:
                 ...
                 if e.stderr: print(e.stderr.decode(errors='ignore'), file=sys.stderr)
                 print("----------------------------", file=sys.stderr)
                 raise # Re-raise to stop execution

        print(f"--- Step 1: Intermediate file created. ---", file=sys.stderr)

        # --- Step 2: Re-encode video, copy audio from temporary file ---
        print(f"\n--- Step 2: Re-encoding video / Copying audio to final file ---", file=sys.stderr)
        ...
        if not mapped_streams:
             raise ValueError("No streams found in intermediate file. Step 1 likely failed.")
        ...
        async with show_progress(duration_step2, desc="Step 2: Final Encode") as progress_url2:
            current_global_args = list(global_args2)
            if progress_url2:
                print(f"Using progress URL for Step 2: {progress_url2}", file=sys.stderr)
                current_global_args.extend(['-progress', progress_url2])
            else:
                 print("Proceeding Step 2 without progress reporting.", file=sys.stderr)

            stream2 = ffmpeg.output(*mapped_streams, output_file, **opts2).global_args(*current_global_args)
            ...
            try:
                ...
                print(f"--- Step 2: FFmpeg process finished. ---", file=sys.stderr)
            except ffmpeg.Error as e:
    ...
    # although errors from await calls will be caught here too.
        ...
        print(f"\nAn unexpected Python error occurred: {type(e).__name__}: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        sys.exit(1)
    finally:
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                ...
                print(f"Cleaned up intermediate files directory: {temp_dir}", file=sys.stderr)
            except OSError as e:
                print(f"Error removing intermediate files directory {temp_dir}: {e}", file=sys.stderr)
