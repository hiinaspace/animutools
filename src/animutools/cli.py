    ...
    try:
        ...
        if args.bulk:
            ...
            if "{num}" not in output_pattern:
                logger.error("Error: Output pattern must contain '{num}' for the episode number.")
                sys.exit(1)

            files_to_process = []
            valid_extensions = (".mkv", ".mp4")

            logger.info(f"Scanning directory: {video_directory} for files ending with {valid_extensions}")
            for filename in os.listdir(video_directory):
                if filename.lower().endswith(valid_extensions):
                    ...
                    if isinstance(episode_number, int):
                        ...
                        except KeyError: # Catching if {num} is not the only placeholder or mistyped
                            logger.error(f"Error: Invalid placeholder in output pattern '{output_pattern}'. Ensure it only uses '{{num}}'.")
                            sys.exit(1)

                        # Output files are in the same directory as input_filepath's directory
                        output_filepath = os.path.join(os.path.dirname(input_filepath), output_filename_part)
                        ...
                        if output_dir_for_file and not os.path.exists(output_dir_for_file):
                            try:
                                ...
                                logger.error(f"Could not create output directory {output_dir_for_file} for {output_filename_part}: {e}. Skipping.")
                                continue
                        ...
                        logger.debug(f"Added to processing list: {filename} -> {output_filepath}, Exists: {output_exists}")
                    else:
                        logger.warning(f"Could not determine episode number for '{filename}'. Skipping.")

            if not files_to_process:
                logger.info("No video files found matching criteria or no episode numbers could be determined. Exiting.")
                sys.exit(0)
            ...
            table.add_column("Guessed Output Filename", style="magenta", overflow="fold", max_width=50)
            ...
            if not args.overwrite:
                proceed_with_encoding = Confirm.ask("Proceed with encoding files marked with ❌ (non-existing)?", default=False)
            ...
            for file_info in files_to_process:
                ...
                logger.info(f"Processing: '{file_info['original_input_filename']}' -> '{os.path.basename(output_f)}'")
                try:
                    ...
                    failed_encodes += 1
                    logger.error(f"Failed to process {file_info['original_input_filename']}: {e}", exc_info=args.verbose)
            ...
            if output_dir and not os.path.exists(output_dir):
                try:
                    ...
                    logger.info(f"Created output directory for single file: {output_dir}")
                except OSError as e:
                    logger.error(f"Could not create output directory {output_dir} for single file: {e}")
                    sys.exit(1)
